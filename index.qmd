...
// 선택된 화학식을 저장할 mutable 변수
mutable selectedFormula = "RAG (검색 증강 생성)";

// 화학식 드롭다운
viewof formulaDropdown = Inputs.select(
  formulas.map(f => ({label: f.name, value: f.name})),
  {label: "화학식 선택", value: "RAG (검색 증강 생성)"}
);

// 드롭다운 변경 감지
formulaDropdownSync = {
  mutable selectedFormula = formulaDropdown;
...
```{ojs}
{
  ...
  if (!current_iso) {
    return html`<div style="padding: 20px;">
      <h3 style="color: #333; font-family: Palatino, Georgia, serif;">
        ${currentElement.symbol} - ${currentElement.name_kr}
      </h3>
      <p style="color: #999; margin-top: 20px;">
        이 원소에 대한 동위원소(구현체) 정보가 아직 등록되지 않았습니다.
      </p>
      <p style="color: #666; font-size: 13px; margin-top: 10px;">
        동위원소는 같은 역할을 수행하지만 프로바이더나 특성이 다른 구체적인 구현체를 의미합니다.
      </p>
    </div>`;
  }
  ...
  return html`<div style="padding: 20px;">
    <h3 style="color: #333; font-family: Palatino, Georgia, serif;">
      ${current_iso.element} - ${current_iso.name}의 동위원소
    </h3>
    <p style="color: #666; font-size: 14px; margin: 15px 0 20px 0;">
      같은 역할을 수행하지만 프로바이더와 특성이 다른 구체적인 구현체들
    </p>
    ...
    ${current_iso.variants.map(v => `
      <div style="margin: 15px 0; padding: 15px; background: #f9f9f9; border-left: 3px solid #666; border-radius: 3px;">
        <h4 style="margin: 0 0 5px 0; color: #333;">
          <sup style="font-size: 12px; color: #666;">${v.provider}</sup>
          <span style="font-size: 18px; margin-left: 5px;">${v.symbol.split('-')[0]}</span>
        </h4>
        <p style="margin: 8px 0 5px 0; font-size: 13px; color: #666;">
          <strong>제품/모델:</strong> ${v.models.join(', ')}
        </p>
...
html`<div style="padding: 20px;">
  <h3 style="color: #333; font-family: Palatino, Georgia, serif; margin-bottom: 15px;">
    동위원소가 등록된 원소들
  </h3>
  <p style="color: #666; font-size: 13px; margin-bottom: 20px;">
    아래 원소들을 클릭하면 해당 동위원소를 확인할 수 있습니다.
  </p>

  <div style="display: flex; flex-wrap: wrap; gap: 10px;">
    ${isotopes.map(iso => {
      const elem = elements.find(e => e.symbol === iso.element);
      return `<button
        onclick="mutable selectedElement = '${iso.element}';"
        style="
          padding: 10px 15px;
          background: ${selectedElement === iso.element ? '#666' : '#f5f5f5'};
          color: ${selectedElement === iso.element ? 'white' : '#333'};
          ...
          transition: all 0.2s;
        "
        onmouseover="this.style.background='${selectedElement === iso.element ? '#555' : '#e8e8e8'}'"
        onmouseout="this.style.background='${selectedElement === iso.element ? '#666' : '#f5f5f5'}'"
      >
        <strong>${iso.element}</strong> - ${elem ? elem.name_kr : iso.element}
        <span style="font-size: 11px; opacity: 0.8;">(${iso.variants.length}개)</span>
      </button>`;
    }).join('')}
  </div>
</div>`
